    ...
    if not file_saved:
      ...
      if reset_flag and not file_saved:
        ...
        if render:
          ...
          # Create renders directory and generate output filename
          renders_dir = Path("renders")
          renders_dir.mkdir(exist_ok=True)
          
          # Extract base name from input file and create output path
          input_name = Path(input_file).stem
          output_video_path = renders_dir / f"{input_name}.mp4"
          ...
          wandb.log({"motion_video": wandb.Video(str(output_video_path), format="mp4")})
